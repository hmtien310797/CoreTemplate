    ...
    namespace BaseCore.Setting
    {
        ...
        public class SettingManager : Singleton<SettingManager>
        {
            ...
            {
                if (!PlayerPrefs.HasKey(KEY_AUDIO)) { Audio = new AudioSettings(); return; }

                var json = PlayerPrefs.GetString(KEY_AUDIO, "");
                if (string.IsNullOrEmpty(json)) { Audio = new AudioSettings(); return; }

                try { Audio = JsonUtility.FromJson<AudioSettings>(json) ?? new AudioSettings(); }
                catch { Audio = new AudioSettings(); }
            }
